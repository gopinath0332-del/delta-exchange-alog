#!/usr/bin/env python3
"""
Donchian Channel 1H Screener — Best Symbol Finder

Runs Donchian Channel (Heikin-Ashi) backtests for ALL 1H coins
and produces a ranked leaderboard with the best symbol to trade.

Usage:
    python scratch/donchian_1h_screener.py
"""
...
# Config
# ---------------------------------------------------------------------------
DATA_FOLDER = r"D:\Workspace\crypto-backtest-data\delta_crypto_data_1H"
TIMEFRAME   = "1h"
LEVERAGE    = 5
MIN_TRADES  = 8
MIN_DATA_ROWS = 500
...
def main():
    print("\n" + "=" * 80)
    print("  DONCHIAN CHANNEL 1H SCREENER")
    ...
    csv_files = sorted(folder_path.glob("*_1h.csv"))
    print(f"\n  Screening {len(csv_files)} 1H files...\n")
    ...
    # Save CSV
    csv_out = OUT_DIR / "donchian_screener_1h.csv"
    df.to_csv(csv_out, index=False)

    # Print leaderboard
    print(f"\n{'='*120}")
    print(f"  1H TIMEFRAME — TOP {TOP_N} DONCHIAN CHANNEL COINS (ranked by composite score)")
    print(f"{'='*120}")
    ...
    print(header)
    print("-" * 120)
    for _, row in df[cols].head(TOP_N).iterrows():
    ...
    # Winner announcement
    winner = df.iloc[0]
    print(f"\n\n  🏆 BEST SYMBOL TO TRADE ON 1H: {winner['Symbol']}")
